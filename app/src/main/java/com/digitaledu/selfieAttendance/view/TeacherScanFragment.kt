        ...
        if (requestCode == 1001) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                if (!shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
                    com.example.selfieAttendance.utility.PermissionUtils.showSettingsDialog(this, "Camera permission is required for face scanning. Please enable it in the app settings.") {
                        parentFragmentManager.popBackStack()
                    }
                } else {
