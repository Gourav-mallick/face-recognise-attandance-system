        ...
        try {
            ...
            if (
                ...
                if (brightness < 40) {
                    ...
                    runOnViewThread {
                        faceGuide.background.setTint(Color.YELLOW)
                        tvStart.text = "Low lighting — please move to a brighter place"
                    }
                    return
                }
        ...
        if (requestCode == 1001) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                if (!shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
                    com.digitaledu.selfieattendance.utility.PermissionUtils.showSettingsDialog(this, "Camera permission is required for face scanning. Please enable it in the app settings.") {
                        parentFragmentManager.popBackStack()
                    }
                } else {
